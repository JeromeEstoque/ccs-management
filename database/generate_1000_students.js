...
function generateSkills(studentId) {
    const skills = [];
    const numSkills = getRandomNumber(3, 8); // Each student has 3-8 skills
    
    // Mix of different skill types
    const skillTypes = [
        { type: 'technical', pool: technicalSkills, count: getRandomNumber(1, 3) },
        { type: 'sports', pool: sportsSkills, count: getRandomNumber(0, 2) },
        { type: 'arts', pool: artsSkills, count: getRandomNumber(0, 2) },
        { type: 'leadership', pool: leadershipSkills, count: getRandomNumber(0, 2) },
        { type: 'other', pool: otherSkills, count: getRandomNumber(0, 2) }
    ];
    
    skillTypes.forEach(({ type, pool, count }) => {
        for (let i = 0; i < count && skills.length < numSkills; i++) {
            const skillName = getRandomElement(pool);
            if (!skills.find(s => s.name === skillName)) {
                skills.push({
                    student_id: studentId,
                    skill_name: skillName,
                    skill_type: type,
                    skill_level: getRandomElement(skillLevels)
                });
            }
        }
    });
...
async function generateStudents() {
    ...
    try {
        ...
        // Get current max student ID to avoid conflicts
        const [maxIdResult] = await connection.execute('SELECT MAX(id) as max_id FROM students');
        ...
        for (let i = 0; i < 1000; i++) {
            ...
            const student = {
                ...
                birth_date: generateBirthdate(yearLevel),
                gender: gender,
            ...
            const studentSkills = generateSkills(studentNum);
            skills.push(...studentSkills);
            
            // Generate work information
            const work = generateWorkInformation(studentNum);
            if (work) {
                workInfo.push(work);
            }
            
            // Generate guardian information
            const guardian = generateGuardianInfo();
            guardians.push({
            ...
            student.guardian_contact = guardian.contact;
        ...
        for (let i = 0; i < students.length; i += batchSize) {
            ...
            const studentValues = batch.map(s => [
                ...
                s.birth_date, s.gender, s.address, s.year_level, s.section, s.gpa, s.status_record,
            ...
            if (batchSkills.length > 0) {
                const skillValues = batchSkills.map(s => [
                    s.student_id, s.skill_name, s.skill_type, s.skill_level
                ]);
                
                const skillQuery = `
                    INSERT INTO student_skills (student_id, skill_name, skill_type, skill_level) 
                    VALUES ?
                `;
        ...
        // Create work information table if it doesn't exist
        await connection.execute(`
            CREATE TABLE IF NOT EXISTS student_work_info (
                ...
                INDEX idx_student_work (student_id)
            )
        `);
        
        // Insert work information
        if (workInfo.length > 0) {
            const workValues = workInfo.map(w => [
                w.student_id, w.company_name, w.position, w.start_date, 
                w.is_current ? null : getRandomDate(2023, 2024), w.is_current, w.hours_per_week
            ]);
            
            const workQuery = `
                INSERT INTO student_work_info (
                    student_id, company_name, position, start_date, end_date, is_current, hours_per_week
                ) VALUES ?
            `;
            
            await connection.query(workQuery, [workValues]);
        }
        
        // Create guardian information table if it doesn't exist
        await connection.execute(`
            CREATE TABLE IF NOT EXISTS guardian_info (
                ...
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                FOREIGN KEY (student_id) REFERENCES students(id) ON DELETE CASCADE,
                INDEX idx_student_guardian (student_id)
            )
        `);
        ...
        if (guardians.length > 0) {
            const guardianValues = guardians.map(g => [
                g.student_id, g.name, g.relationship, g.contact, g.email, 
                generateAddress(), getRandomElement(['Engineer', 'Teacher', 'Doctor', 'Business Owner', 'Government Employee', 'Retired', 'Nurse', 'Accountant'])
            ]);
            
            const guardianQuery = `
                INSERT INTO guardian_info (
                    student_id, guardian_name, relationship, contact, email, address, occupation
                ) VALUES ?
            `;
            
            await connection.query(guardianQuery, [guardianValues]);
        }
        ...
        await connection.execute(`
            UPDATE system_statistics SET 
                ...
                average_gpa = (SELECT AVG(gpa) FROM students WHERE gpa IS NOT NULL),
                updated_at = CURRENT_TIMESTAMP
        `);
        
        console.log(`Successfully generated and inserted ${students.length} students with complete information!`);
        console.log(`- ${skills.length} skills entries`);
        console.log(`- ${workInfo.length} work information entries`);
        console.log(`- ${guardians.length} guardian information entries`);
        ...
        const summary = {
            ...
            average_gpa: (students.reduce((sum, s) => sum + parseFloat(s.gpa), 0) / students.length).toFixed(2),
            students_with_work: workInfo.length,
