// Script to add remaining students to reach ~1000 total (fixed version)
...
async function addRemainingStudents() {
    const connection = await mysql.createConnection(dbConfig);
    
    try {
        console.log('Adding remaining students to reach ~1000 total...');
        
        // Get current stats with separate queries
        const [maxStudentResult] = await connection.execute('SELECT MAX(id) as max_student_id FROM students');
        const [maxUserResult] = await connection.execute('SELECT MAX(id) as max_user_id FROM users');
        const [countResult] = await connection.execute('SELECT COUNT(*) as current_count FROM students');
        
        const currentMaxStudentId = maxStudentResult[0].max_student_id || 0;
        const currentMaxUserId = maxUserResult[0].max_user_id || 0;
        const currentCount = countResult[0].current_count;
        
        const startId = Math.max(currentMaxStudentId, currentMaxUserId) + 1;
        const targetCount = 1000;
        const studentsToAdd = targetCount - currentCount;
        
        console.log(`Current student count: ${currentCount}`);
        console.log(`Target student count: ${targetCount}`);
        ...
        const [existingUsers] = await connection.execute('SELECT username, email FROM users');
        const existingUsernames = new Set(existingUsers.map(u => u.username));
        const existingEmails = new Set(existingUsers.map(u => u.email).filter(e => e));
        ...
        for (let i = 0; i < studentsToAdd; i++) {
            ...
            const course = getRandomElement(allCourses);
            ...
            do {
                ...
                emailCounter++;
            } while (existingEmails.has(email) && emailCounter < 100);
        ...
        console.log(`Successfully added ${users.length} users and ${students.length} students!`);
        
        // Verify final counts
        const [finalStudentCount] = await connection.execute('SELECT COUNT(*) as total_students FROM students');
        const [finalUserCounts] = await connection.execute('SELECT role, COUNT(*) as count FROM users GROUP BY role');
        const [finalCourseCounts] = await connection.execute('SELECT SUBSTRING(section, 1, 4) as course, COUNT(*) as count FROM students GROUP BY SUBSTRING(section, 1, 4) ORDER BY count DESC LIMIT 10');
        
        console.log('\n=== Final Database State ===');
        console.log(`Total Students: ${finalStudentCount[0].total_students}`);
        console.log('User Distribution:');
        finalUserCounts.forEach(row => {
            console.log(`  ${row.role}: ${row.count}`);
        });
        console.log('Top 10 Programs:');
        finalCourseCounts.forEach(row => {
            console.log(`  ${row.course}: ${row.count} students`);
        });
        
        console.log('\n✅ Student restoration completed successfully!');
        console.log('✅ System now has ~1000 students as originally intended!');
