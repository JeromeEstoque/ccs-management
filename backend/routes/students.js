...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    if (search) {
      // Split search term to handle first name + last name searches
      const searchTerms = search.trim().split(/\s+/);
      if (searchTerms.length >= 2) {
        // If multiple words, search for first word in first_name and second word in last_name
        query += " AND (s.first_name LIKE ? AND s.last_name LIKE ?)";
        params.push(`%${searchTerms[0]}%`, `%${searchTerms[1]}%`);
      } else {
        // Single word search
        query +=
...
router.get("/user/:userId", authenticateToken, async (req, res) => {
  try {
    ...
    // Check access - admin can view any profile, student can only view their own profile
    if (req.user.role === "admin") {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Access denied - teachers cannot view student profiles",
        });
    }
    ...
    try {
      ...
      if (!studentId || !password || !firstName || !lastName) {
        return res.status(400).json({
          success: false,
          message:
            "Student ID, password, first name, and last name are required",
        });
      }
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (yearLevel && req.user.role !== "student") {
      ...
      if (statusRecord && req.user.role === "admin") {
        updates.push("status_record = ?");
        values.push(statusRecord);
      }
      if (lastSchoolAttended !== undefined) {
        updates.push("last_school_attended = ?");
        values.push(lastSchoolAttended);
      }
      ...
      if (medicalRecords !== undefined) {
        updates.push("medical_records = ?");
        values.push(medicalRecords);
      }
      if (workingStudent !== undefined) {
        updates.push("working_student = ?");
        values.push(workingStudent);
      }
      ...
      if (organizationRole !== undefined) {
        updates.push("organization_role = ?");
        values.push(organizationRole);
      }
      if (classRepresentative !== undefined) {
        updates.push("class_representative = ?");
        values.push(classRepresentative);
      }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      await pool.query("UPDATE students SET profile_picture = ? WHERE id = ?", [
        fileUrl,
        id,
      ]);
...
router.post(
  ...
  authenticateToken,
  authorizeRoles("admin"),
  auditLog("RESET_STUDENT_PASSWORD"),
  async (req, res) => {
    try {
      const { id } = req.params;
...
router.get("/skills/list", authenticateToken, async (req, res) => {
  try {
    const [skills] = await pool.query(
      "SELECT * FROM skills ORDER BY type, name",
    );
