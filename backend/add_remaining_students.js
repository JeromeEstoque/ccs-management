// Script to add remaining students to reach ~1000 total
...
async function addRemainingStudents() {
    ...
    try {
        ...
        // Get current max IDs and count
        const [currentStats] = await connection.execute('SELECT MAX(id) as max_student_id FROM students; SELECT MAX(id) as max_user_id FROM users; SELECT COUNT(*) as current_count FROM students;');
        const currentMaxStudentId = currentStats[0][0].max_student_id || 0;
        const currentMaxUserId = currentStats[1][0].max_user_id || 0;
        const currentCount = currentStats[2][0].current_count;
        ...
        // Verify final counts
        const [finalCounts] = await connection.execute('SELECT COUNT(*) as total_students FROM students; SELECT role, COUNT(*) as count FROM users GROUP BY role; SELECT SUBSTRING(section, 1, 4) as course, COUNT(*) as count FROM students GROUP BY SUBSTRING(section, 1, 4) ORDER BY count DESC LIMIT 10;');
